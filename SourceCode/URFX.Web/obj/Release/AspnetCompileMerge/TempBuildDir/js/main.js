...
//requirejs(["FileSaver.min","jspdf.debug"], function (obj) { });

angular.module('app')
//directive for items per page

  .controller('AppCtrl', ['$scope', '$translate', '$localStorage', '$window', 'authService', '$state', 'ngAuthSettings', '$cookies','$rootScope',
    function ($scope, $translate, $localStorage, $window, authService, $state, ngAuthSettings, $cookies,$rootScope) {
        // add 'ie' classes to html
        $rootScope.showLoader = false;
        $rootScope.searchNewJobText;
        ...
        $scope.app = {
            ...
            currentUser: {
                ...
                userRole:loadCurrentUser().userRole,
                userId:loadCurrentUser().userId,
                planId:loadCurrentUser().planId,
                type:loadCurrentUser().type,
            },
        ...
        $scope.logout = function () {
           authService.logOut($scope.app.currentUser.userId).then(function (response) {
                     $scope.currentUser = [];
                     $state.go('signin');
                }, function (response) {
                });          
